...
export class Attribute extends Behaviour {
    ...
    onStart() { //基础数值
        ...
        this.gameObject.getBehaviour(TextRenderer).text= this.value + "%";
    }
    ...
    onTick(duringTime: number) {
        ...
        if(this.nowtime-this.lasttime >=1){
            this.value = this.changevalue(this.value,-5);
    ...
    changevalue(totalevalue:number,changedvalue:number){ //改变value 且value最大值最小值不能超过最大值最小值
